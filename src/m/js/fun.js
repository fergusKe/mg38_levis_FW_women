...
var Fun = (function($) {
    ...
    funObj.tracking = function() {
      $('[megais_ga]').each(function() {
        let $this = $(this)

        if ($this.data('event') == 'tracking') return

        $this.data('event', 'tracking')

        $this.on('click', function(e) {
          // e.preventDefault()
          console.log('click')
          let trackStr = $(this).attr('megais_ga')
          window.trackingEvent(trackStr, "click")
        })

        // 確認有沒有下到tracking
        // console.log($('[megais_ga]').length)
        // $('[megais_ga]').each(function() {
        // 	let tracking = $(this).data('event')
    ...
    funObj.popupChange = function(pEle, pBol, pComplete) {
      pBol = pBol || false;
  		if (pBol) {
  			...
  			Fun.eleFadeOut(pEle, 'normal', pComplete);
  		}
